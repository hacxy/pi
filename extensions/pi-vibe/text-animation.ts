...
const GLOBAL_TEXT_TIMER_KEY = "__pi_vibe_text_animation_timer__";
...
{
	const staleId = (globalThis as any)[GLOBAL_TEXT_TIMER_KEY];
	if (staleId != null) {
		clearInterval(staleId);
		delete (globalThis as any)[GLOBAL_TEXT_TIMER_KEY];
	}
}
...
function applyBreatheAnimation(ctx: ExtensionContext, message: string): void {
	...
	const tid = setInterval(() => {
		...
		ctx.ui.setWorkingMessage(
			`\x1b[38;2;${r};${g};${b}m${message}\x1b[0m ${getTimeStr()}`,
		);
		phase += 0.15;
	}, 50);
	textAnimationTimer = tid;
